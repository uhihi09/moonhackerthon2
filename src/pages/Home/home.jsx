...
const Home = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (emergencyRes.data && emergencyRes.data.records) {
        setEmergencyButtonRecords(emergencyRes.data.records);
      }
    } catch (error) {
      console.error('데이터 로드 실패:', error);
      setCurrentLocation({
        address: '경상남도 사천시 곤명리',
  ...
  const formatEmergencyDate = (timestamp) => {
    try {
...
<MapLocationCard>
  ...
  <LocationHistoryButton onClick={handleLocationHistoryClick}>
    ...
    <ArrowIcon>›</ArrowIcon>
        ...
        {/* 위급버튼 기록 카드 */}
        <EmergencyButtonCard>
          <CardTitle>위급버튼 기록</CardTitle>
          {emergencyButtonRecords.length === 0 ? (
            <EmptyMessage>
              최근 위급버튼을 사용한 기록이 없어요.
            </EmptyMessage>
          ) : (
            <EmergencyRecordList>
              {emergencyButtonRecords.map((record, index) => (
                <React.Fragment key={record.id}>
                  ...
                  {index < emergencyButtonRecords.length - 1 && <RecordDivider />}
                </React.Fragment>
              ))}
            </EmergencyRecordList>
          )}
        ...
        {/* 긴급신고 섹션 */}
        <EmergencySection>
          <SectionTitle>긴급신고</SectionTitle>
          <EmergencyButtons>
            <EmergencyButton 
              variant="police"
...
const MapLocationCard = styled.div`
  background: white;
  border-radius: 16px;
  overflow: hidden;
...
const LocationHistoryButton = styled.button`
  display: flex;
  align-items: center;
  gap: 12px;
  background: white;
...
const EmergencyButtonCard = styled.div`
  background: white;
  border-radius: 16px;
  padding: 24px;
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
...
const EmergencyRecordList = styled.div`
  display: flex;
  flex-direction: column;
`;

const EmergencyRecordItem = styled.button`
  display: flex;
  align-items: center;
  gap: 16px;
  padding: 20px 0;
...
const RecordDateTime = styled.span`
  flex: 1;
  font-size: 16px;
  font-weight: 600;
  color: #333;
...
const EmergencyButton = styled.button`
  ...
  box-shadow: ${props => props.variant === 'police' 
    ? '0 2px 12px rgba(59, 130, 246, 0.25), 0 0 20px rgba(59, 130, 246, 0.15)' 
    : '0 2px 12px rgba(239, 68, 68, 0.25), 0 0 20px rgba(239, 68, 68, 0.15)'};

  &:hover {
    background: ${props => props.variant === 'police' ? '#EFF6FF' : '#FEF2F2'};
    transform: translateY(-2px);
    box-shadow: ${props => props.variant === 'police' 
      ? '0 4px 16px rgba(59, 130, 246, 0.3), 0 0 30px rgba(59, 130, 246, 0.2)' 
      : '0 4px 16px rgba(239, 68, 68, 0.3), 0 0 30px rgba(239, 68, 68, 0.2)'};
  }
